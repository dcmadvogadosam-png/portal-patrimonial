...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const pessoas_moram_junto = normalizarPessoasMoramJunto(body?.pessoas_moram_junto || body?.moradores_junto || "");
    ...
    const createUserRes = await supabaseFetch(env, "/auth/v1/admin/users", {
      ...
      body: JSON.stringify({
        ...
        password,
        email_confirm: true,
        user_metadata: { nome, role: "morador", unidade, condominio_id, cpf, celular, placa_veiculo, pessoas_moram_junto, foto_placa_veiculo_url }
      })
    });
