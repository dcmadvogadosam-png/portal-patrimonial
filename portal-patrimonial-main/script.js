...
function renderMoradoresTable(){
  ...
  const rows=moradores.filter(m => [m.nome,m.email,m.unidade,m.cpf,m.placa_veiculo].join(" ").toLowerCase().includes(busca)).map(m=>{
    ...
    return `<tr><td><strong>${escapeHtml(m.nome||"-")}</strong></td><td>${escapeHtml(m.email||"-")}</td><td>${escapeHtml(m.unidade||"-")}</td><td>${escapeHtml(cond)}</td><td>${escapeHtml(m.placa_veiculo||"-")}</td><td>${foto}</td><td>${m.ativo===false?"Inativo":"Ativo"}</td></tr>`;
  }).join("");
  tbody.innerHTML = rows || `<tr><td colspan="7">Nenhum morador cadastrado.</td></tr>`;
}
...
async function criarMorador(e){
  ...
  try{
    ...
    if(!res.ok) return msg($("adminMsg"), result.error || `Erro ao criar morador. Status: ${res.status}. Verifique /api/create-user e as variáveis do Cloudflare.`, "error");
